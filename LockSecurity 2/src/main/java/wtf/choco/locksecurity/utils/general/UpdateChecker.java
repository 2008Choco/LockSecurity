package wtf.choco.locksecurity.utils.general;
...
public class UpdateChecker {
	 ...
	 * 
	 * @param plugin the plugin to check
	 * @param pluginID the plugin id (found on Spigot). Must be an integer
	 */
	...
	public boolean queryUpdateCheck() {
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(new URL(queryURL).openStream()))) {
			...
			JsonObject root = LockSecurity.GSON.fromJson(reader, JsonArray.class).get(0).getAsJsonObject();
